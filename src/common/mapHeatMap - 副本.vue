...
// import '../../static/example/olGisMap/baseMap.js';
// import {getLayer,getLayers} from  '../../static/example/olGisMap/baseMap.js';
...
export default {
  ...
  mounted () {
    // this.$nextTick(function () {
      this.init();
      this.addHotMapLayer(Map,this.hotData)
    // })
  },
  methods: {
    //地图初始化
    ...
    dataUpdate (geo) {
      this.$nextTick(function () {
        if (geo.center != this.mapData.center)
          this.map.getView().setCenter(fromLonLat(geo.center));
        if (geo.zoom != this.mapData.zoom)
          this.map.getView().setZoom(geo.zoom);
        if (geo.minZoom != this.mapData.minZoom)
          this.map.getView().setMinZoom(geo.minZoom);
        if (geo.maxZoom != this.mapData.maxZoom)
     ...
     */
     addHotMapLayer(map, hotData) {
        ...
        // if (!this.map.getLayer(map, "gisMap")) {
        if (!this.vector) {
            ...
            this.vector.setZIndex(39);
            // map.addLayer(this.vector);
            this.map.addLayer(this.vector);
        } else {
            this.vector = this.map.getLayer(map, "gisMap");
        }
